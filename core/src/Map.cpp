    ...
    void Map::setTileCoords()
    {
        ...
        for (int y = 0; y < mMapDimensions.y; y++)
        {
            for (int x = 0; x < mMapDimensions.x; x++)
            {
                ...
                posCoords[3] = (y + 1) * mTileSize - border; // Bottom left y
                // P3
                posCoords[4] = (x + 1) * mTileSize - border; // Top right x
                posCoords[5] = y * mTileSize; // Top right y
                // P4
                posCoords[6] = (x + 1) * mTileSize - border; // Bottom right x
                posCoords[7] = (y + 1) * mTileSize - border;  // Bottom right y
